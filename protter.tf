...
; - Added Pfe upmessage from two towers helmet                        ;
;                                                                     ;
; Changes to 1.3.35                                                   ;
; - Fixed Soul shield a bit                                           ;
;                                                                     ;
; Changes to 1.3.34                                                   ;
...
/set protavgfile=~/build/tf-lib/protavg.times
...
/def createprot=\
 /if (!getopts("w:r:p:n:u:d:t:sc", "0")) /break%;/endif%;\
  ...
  /set pname=%opt_p%;\
  /if (!opt_c) \
   /def -i -p90 -aBCblue -t'%{opt_u}' %{opt_n}_up=\
   /let timenow=$$[time()]%%; \
...
/createprot -t0 -n"MS" -w"{*} * \'nullum driiiks umbah mana\'" -u"You feel your magical power expanding." -d"Your life force seems weaker." -p"Mana Shield"
/createprot -t0 -n"RT" -w"{*} * \'qxx'rzzzz\'" -u"* you are surrounded by twinkling aura." -d"You feel more vulnerable against flips." -p"Resist Temptation"
